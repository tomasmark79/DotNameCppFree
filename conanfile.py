...
class ProjectTemplateRecipe(ConanFile):
    ...
    def generate(self): 
        ...
        tc.generate()
        # Update presets after they are generated by CMakeToolchain
    ...
    def update_cmake_presets(self, preset_file):
        ...
        if not os.path.exists(preset_file):
            self.output.info(f"Preset file {preset_file} does not exist yet, skipping preset update")
            return
        ...
        try:
            ...
            # Update build presets - only update name and configurePreset reference
            for preset in data.get("buildPresets", []):
                configure_preset_ref = preset.get("configurePreset", "")
                if configure_preset_ref in name_mapping:
                    # Update the name to match the configure preset
                    preset["name"] = f"{preset['name']}-{build_suffix}"
                    # Update the reference to the configure preset
                    preset["configurePreset"] = name_mapping[configure_preset_ref]
                    
            # Update test presets - only update name and configurePreset reference  
            ...
            self.output.info(f"Updated preset names in {preset_file} with suffix: {build_suffix}")
            
        except (json.JSONDecodeError, KeyError, IOError) as e:
            self.output.warn(f"Failed to update CMake presets in {preset_file}: {e}")
